...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('runs/imgpgd')

    train_set = TrainDatasetFromFolder('data/train/', crop_size=CROP_SIZE,
                                       upscale_factor=UPSCALE_FACTOR,
                                       batch_size=batch_size)
    val_set = ValDatasetFromFolder('data/test/',
                                   upscale_factor=UPSCALE_FACTOR)
    train_loader = DataLoader(dataset=train_set, num_workers=12, batch_size=batch_size, shuffle=True)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for data, target in train_bar:  # train epoch
            ...
            if (epoch == 20 or epoch == 60) and cur_grow < opt.max_grow and epoch_flag == 0:
        ...
        torch.save(netG.state_dict(), 'epochs/imgpgd/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
        torch.save(netD.state_dict(), 'epochs/imgpgd/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
